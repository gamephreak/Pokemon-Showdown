			...
			for (const moveset of analysis.movesets) {
				...
				for (const battleOnlyForme of battleOnlyFormes) {
					if (typeof battleOnlyForme.battleOnly !== 'string') {
						...
						addSmogonSet(dex, format, battleOnlyForme.name, disambiguated, set, setsForPokemon, numByFormat, pokemon);
					} else if (battleOnlyForme.battleOnly === pokemon) {
						addSmogonSet(dex, format, battleOnlyForme.name, name, set, setsForPokemon, numByFormat);
					}
				}
			}
		}
